...
function html(dom, options) {
  ...
  const opts = {
    ...
    ...flatten(options !== null && options !== undefined ? options : {})
  };
...
function parseHTML(data, context, keepScripts = typeof context === "boolean" ? context : false) {
  ...
  const parsed = this.load(data, options_default, false);
  if (!keepScripts) {
...
function cloneDom(dom) {
  ...
  const root2 = new Document(clone);
  clone.forEach((node2) => {
    node2.parent = root2;
  });
  return clone;
...
class Parser2 {
  constructor(cbs, options = {}) {
    ...
    this.tokenizer = new ((_c = options.Tokenizer) !== null && _c !== undefined ? _c : Tokenizer2)(this.options, this);
...
function stringEncaseCRLFWithFirstIndex(string, prefix, postfix, index2) {
  ...
  do {
    ...
    returnValue += string.slice(endIndex, gotCR ? index2 - 1 : index2) + prefix + (gotCR ? `\r
` : `
`) + postfix;
    endIndex = index2 + 1;
...
program2.name("seo-tech-check").arguments("<dir>").description("Static SEO audit for legacy projects using schema.org, meta tags, favicons, and more").action(async (dir) => {
  ...
  const entries = await import_fast_glob.default(["**/*.html", "**/*.twig", "**/*.php"], {
    cwd: base,
    onlyFiles: true,
    dot: true,
    absolute: true
  ...
  if (files.length === 0) {
    console.log(source_default.yellow("⚠️ No files found for analysis."));
    process.exit(1);
  }
  const metaTests = {
    doctype: false,
  ...
  const re = {
    ...
    favicons: /<link[^>]*rel=['"](?:icon|apple-touch-icon|shortcut icon|manifest)['"][^>]*>|<meta[^>]*name=['"]msapplication-TileImage['"][^>]*>/i,
  ...
  for (const file of files) {
    ...
    if (!socialTests.twitterDescription && re.twitterDescription.test(content))
      socialTests.twitterDescription = true;
    const $2 = esm_default2.load(content);
    $2("img").each((_, el) => {
  ...
  const okIcon = source_default.green("✅");
  const warnIcon = source_default.yellow("⚠️");
  const failIcon = source_default.red("❌");
  function printGroup(name, tests) {
    const keys = Object.keys(tests);
    const passed = keys.filter((k) => tests[k]).length;
    if (passed === keys.length) {
      console.log(`${okIcon} ${source_default.bold(name)}`);
    } else if (passed === 0) {
      console.log(`${failIcon} ${source_default.bold(name)}`);
    } else {
      console.log(`${warnIcon} ${source_default.bold(name)}`);
    }
    if (passed < keys.length) {
  ...
  console.log(source_default.underline("SEO Audit Report:"));
  ...
  if (imagesMissing.size === 0) {
    console.log(`${okIcon} ${source_default.bold("Images")} (all <img> tags have alt attributes)`);
  } else {
    console.log(`${warnIcon} ${source_default.bold("Images")} (missing alt attributes)`);
